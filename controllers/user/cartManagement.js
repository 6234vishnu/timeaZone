...
const cartPage = async (req, res) => {
    try {
      ...
      const cart = await Cart.findOne({userId:id}).populate('items.product').exec();
    
      const product=await Product.findOne({userId:id})
      const adress=await Address.findOne({userId:id})||{}
  
      res.render("cartUserside",{user:userData,cart:cart|| { items: [] },product:product,adress:adress});
      ...
      console.log("error in cartPage ", error);
    }
  };
  ...
  const removeFromCart = async (req, res) => {
    try {
      ...
      console.log('User cart items:', cartDetails.items);
      cartDetails.items = cartDetails.items.filter(item => item._id.toString() !== itemId);

      cartDetails.totalAmount = cartDetails.items.reduce((total, item) => total + (item.price * item.quantity), 0);
  ...
  const updateCartQnty = async (req, res) => {
    try {
      console.log("Item and quantity", req.body);
      const { itemId, quantity } = req.body;
  
      if (req.session.user || req.session.googleId) {
        const cartDetails = await Cart.findOne({ userId: req.session.user?._id || req.session.googleId });
        ...
        const cartItem = await Cart.findOne(
          { userId: req.session.user?._id || req.session.googleId, "items.product": itemId },
          { "items.$": 1 }
        );
        ...
        if (!productInCart) {
          return res.status(400).json({ success: false, message: 'Product information is missing in cart item' });
        }
        ...
        if (quantity > product.quantity) {
          return res.status(400).json({ success: false, message: 'Stocks unavailable' });
        }
  
        // Update the quantity of the product in the cart
        const updatedQuantity = await Cart.updateOne(
          { "items.product": product._id, userId: req.session.user?._id || req.session.googleId },
          { $set: { "items.$.quantity": quantity } }
        );
  
        if (updatedQuantity.modifiedCount > 0) {
          // Recalculate the subtotal
          const updatedCart = await Cart.findOne({ userId: req.session.user?._id || req.session.googleId });
          ...
          return res.json({ success: true, message: 'Cart and product updated successfully', subtotal: subtotal });
        }
      }

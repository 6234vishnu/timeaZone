...
const loadProduct = async (req, res) => {
    try {
      ...
      console.log('google',req.session.googleId)
        ...
        const similarProducts = await Product.find({
          ...
          let userData=null
          if(req.session.user){
          userData= await User.findOne({_id:req.session.user,isBlocked:false})
          if(userData){
            req.session.user=userData
          }
          }
          if(req.session.googleId){
            userData= await User.findOne({_id:req.session.googleId,isBlocked:false})
            if(userData){
         ...
         const coupons=await Coupon.find({applicableCategories:product.category.name})
      ...
      console.log("Error in loadProduct:", error);
    }
  };
  ...
  const shopPage = async (req, res) => {
    try {
      ...
      const product = await Product.find({ isBlocked: false }).skip(skip).limit(limit);
      ...
      let userData = null;
      if (req.session.user) {
        userData = await User.findOne({ _id: req.session.user._id, isBlocked: false });
        if (userData) {
          req.session.user = userData;
        }
      }
  ...
  const filterProducts = async (req, res) => {
    try {
      ...
      const { brand, category, priceRange, sortBy, page = 1, limit = 9 } = req.query;
      ...
      if (brand) {
        ...
        console.log("Brand filter:", brandDoc);
        if (brandDoc) {
      ...
      if (category) {
        ...
        console.log("Category filter:", categoryDoc);
        if (categoryDoc) {

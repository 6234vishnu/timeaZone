...
try {
  ...
  const totalPages = Math.ceil(totalCoupons / limit);
    ...
    res.render('couponAdmin',{coupon:coupon,currentPage:page,totalPages})
    
} catch (error) {
    console.log('error in coupon render admin',error)
...
const couponCreateRender=async(req,res)=>{
    try {
      const product=await Product.find({isBlocked:false})
        ...
        console.log('error in Create coupon render admin')
...
const couponCreate=async(req,res)=>{
    try {
      ...
      if(discountValue<=0){
        ...
        if (existingCoupon) {
            return res.status(400).render("createCoupon",{ message: 'Coupon with this code already exists' });
        }

        const selectedProduct=await Product.findOne({name:product})
        if(!selectedProduct){
          return  res.status(400).render("createCoupon",{ message: 'product not found' });
        }
...
const blockCoupon = async (req, res) => {
    try {
      ...
      console.log("Error in blockCoupon:", error);
    }
  };
  const unBlockCoupon = async (req, res) => {
    try {
      ...
      res.status(500).render("pageNotfoundServer");
      console.log("Error in unblockCoupon:", error);
    }
  };

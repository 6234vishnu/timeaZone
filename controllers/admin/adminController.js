...
const loadLoginPage = async (req, res) => {
  try {
    ...
    if (error) {
      console.log("error in loginpage adminside");
    }
  }
};
...
const login = async (req, res) => {
  try {
    ...
    if (!admin) {
      ...
      return res.render("adminLogin", { message: 'Incorrect email or not an admin' });
    }
    ...
    return res.render("adminLogin", { message: "Error in login, please try again" });
  }
};
...
const adminlogout = async (req, res) => {
  try {
      ...
      req.session.destroy((err) => {
        if (err) {
          ...
          res.setHeader('Pragma', 'no-cache'); 
          res.setHeader('Expires', '0'); 
          return res.render("adminDashboard", { message: "An error occurred, can't log out" });
        }else {
      ...
      res.render("adminDashboard", { message: "An unexpected error occurred, can't log out" });
   };
  }
  ...
  const forgotPasswordEmail = async (req, res) => {
    try {
      ...
      if (error) {
        ...
        console.log("error in foregot password",error);
      }
    }
  };
  ...
  const otpcompareforgotAdmin=async(req,res)=>{
    ...
    try {
      ...
      console.log('generetedOtp',generatedOtp);
      const generatedOtp=req.session.otpGenerated
    
      
    
      if(otp===generatedOtp){
       return res.status(200).json({success: true, redirectUrl:'/admin/newPasswordAdmin'})
      }else{
    ...
    const loadDashboard = async (req, res) => {
      try {
        ...
        if (error) {
          ...
          console.log("error to render admoindashboard");
        }
      }
    };
    ...
    function getAllDatesInMonth(year, month) {
      ...
      for (let day = 1; day <= daysInMonth; day++) {
        dates.push(`${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`); // Format as YYYY-MM-DD
      }
      return dates;
    }
    ...
    function getWeeksInMonth(year, month) {
      ...
      while (currentWeekStart <= lastDate) {
        ...
        weeks.push({
          ...
          end: weekEnd <= lastDate ? weekEnd.toISOString().split('T')[0] : lastDate.toISOString().split('T')[0]
        });
  ...
  try {
    ...
    if (aggregation === 'daily') {
      ...
      return res.status(400).json({ message: 'Invalid aggregation type. Use daily, weekly, or monthly.' });
    }
